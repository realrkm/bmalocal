from ._anvil_designer import DefectsForm_copyTemplate
from anvil import *
import anvil.server
import anvil.users
...
class DefectsForm_copy(DefectsForm_copyTemplate):
    ...
    def populateForm(self, defects_data, **event_args):
        ...
        if defectListData:
            ...
            self.txtTechnicianPortalRequestedParts.text = defectListData[0][
                "TechnicianPortalRequestedParts"
            ]
        ...
        if not self.txtClientInstructions.text:
            getJobCardDetails = anvil.server.call(
                "getJobCardRow", defects_data[0]["ID"]
            )
    ...
    def btn_Update_click(self, **event_args):
        ...
        if not existingrecord:
            ...
            anvil.server.call(
                "saveTecnicianDefectsAndRequestedParts",
                jobcardID,
                defects,
                priceddefects,
                parts,
    ...
    def btn_DownloadTechNotes_click(self, **event_args):
        ...
        jobcardIDWithTechNotes = anvil.server.call(
            "getTechNotes", self.defects_data[0]["ID"]
        )
        ...
        val_to_check = (
            jobcardIDWithTechNotes.strip()
            if isinstance(jobcardIDWithTechNotes, str)
            else jobcardIDWithTechNotes
        )
    ...
    def downloadTechNotesPdf(self, job_card_id):
        ...
        if result is None:
            ...
            media_object = anvil.server.call(
                "downloadTechNotesPdfForm", job_card_id, "TechNotes"
            )
    ...
    def downloadDefectsPdf(self, job_card_id):
        ...
        if result is None:
            ...
            media_object = anvil.server.call(
                "downloadDefectsPdfForm", job_card_id, "DefectsList"
            )
            anvil.media.download(media_object)
    ...
    def downloadPricedDefectsPdf(self, job_card_id):
        ...
        result = anvil.server.call(
            "get_priced_defects_list_details_by_job_id", job_card_id
        )
        if result is None:
            anvil.alert(
                "No data was returned from the server. Please confirm the defects data exists.",
            ...
            media_object = anvil.server.call(
                "downloadDefectsPdfForm", job_card_id, "PricedDefectsList"
            )
            anvil.media.download(media_object)
    ...
    def drop_down_selectPart_change(self, **event_args):
        """This method is called when an item is selected"""
        self.txtSellingPrice.text = ModGetData.getSellingPrice(
            self.drop_down_selectPart.selected_value
        )
    ...
    def btn_IssueInvoice_click(self, **event_args):
        ...
        anvil.server.call(
            ...
            self.defects_data[0]["ID"],
            f"ready for invoicing - {now_str}",
        )
    ...
    def btn_IncompleteDefectsInfo_click(self, **event_args):
        """This method is called when the button is clicked"""
        anvil.server.call(
            "publish_defects_notification",
            self.defects_data[0]["ID"],
            "defects list incomplete",
        )
    ...
    def btn_AddMorePartsAndServicesInConfirmedQuote_click(self, **event_args):
        ...
        if (
            ...
            or result["Status"] == "Verify Task"
            or result["Status"] == "Issue Invoice"
        ):
            alert(
                content=AddMorePartsInConfirmQuote(self.defects_data[0]["ID"]),
                buttons=[],
                dismissible=False,
                large=True,
            )
            ...
            alert(
                "Sorry, you can only update In Service, Verify Task or Issue Invoice jobcards.",
