(() => {
            ...
            ${state.categories.map(c => {
                ...
                return `
                    <button onclick="selectModalCategory('${c.id}')" class="category-card ${c.color}" aria-label="View ${sanitizeHTML(c.name)} category">
                        ...
                        <span style="font-size:1.8rem; color:#000000;">
            ...
            <div style="background:linear-gradient(135deg, rgba(30, 41, 59, 0.8) 0%, rgba(15, 23, 42, 0.8) 100%); backdrop-filter:blur(10px); padding:3rem; border-radius:2rem; border:2px solid rgba(129, 196, 255, 0.3); box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3);">
                ...
                <label for="workdone-textarea" style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem; font-size:2rem; font-weight:bold; color:#16588e;">
                    ...
                    <div style="display:flex; gap:0.5rem;">
                        ...
                        <button id="workdone-voice-start" type="button" style="background:linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color:white; padding:0.4rem 0.8rem; border-radius:0.5rem; border:2px solid rgba(34, 197, 94, 0.3); font-weight:bold; cursor:pointer; font-size:2.2rem;"><i data-lucide="mic"></i> </button>
    ...
    function parseRequestedPartsText(text) {
        ...
        for (const line of lines) {
            ...
            // Parse format: "1. Part Name (Category) - Qty: X"
            // or: "Part Name (Category) - Qty: X"
    ...
    function renderParts(arr) {
        return arr.map((p, index) => `<div style="background:linear-gradient(135deg, rgba(51, 65, 85, 0.8) 0%, rgba(30, 41, 59, 0.8) 100%); backdrop-filter:blur(10px); padding:2rem; border-radius:1.5rem; display:flex; flex-direction:column; justify-content:space-between; gap:1.5rem; text-align:center; border:2px solid rgba(129, 196, 255, 0.2); transition:all 0.3s ease; box-shadow: 0 10px 20px rgba(0, 0, 0, 0.3);" onmouseover="this.style.transform='translateY(-5px)'; this.style.borderColor='rgba(129, 196, 255, 0.5)'; this.style.boxShadow='0 15px 30px rgba(129, 196, 255, 0.3)';" onmouseout="this.style.transform=''; this.style.borderColor='rgba(129, 196, 255, 0.2)'; this.style.boxShadow='0 10px 20px rgba(0, 0, 0, 0.3)';"> <div> <h3 style="font-size:2rem; color:#16588e;">${sanitizeHTML(p.name)}</h3> <p style="color:#94a3b8; margin-top:0.5rem">Category: ${sanitizeHTML(p.category || 'N/A')}</p> </div> <div style="display:flex; flex-direction:column; gap:1rem; align-items:center;"> <div style="width:100%;"> <label for="qty-${index}" style="display:block; color:#94a3b8; font-size:1.4rem; margin-bottom:0.5rem;">Quantity</label> <input type="number" id="qty-${index}" min="0" step="0.01" value="1" placeholder="0.00" aria-label="Quantity for ${sanitizeHTML(p.name)}" style="width:100%; padding:0.8rem; font-size:1.6rem; border-radius:0.8rem; border:2px solid rgba(129, 196, 255, 0.3); background:linear-gradient(135deg, rgba(30, 41, 59, 0.8) 0%, rgba(15, 23, 42, 0.8) 100%); color:white; text-align:center;" > </div> <button onclick="addToCart('${p.name.replace(/'/g, "\\'")}', '${index}')" class="add-btn-circular" aria-label="Add ${sanitizeHTML(p.name)} to cart">+</button> </div> </div>`).join('');
    }
    ...
    // ===========================
    // ANIMATED BACKGROUND (website_background.gif)
    // ===========================
    const BACKGROUND_SRC = '_/theme/website_background.gif';
    let backgroundPreloadPromise = null;
    let backgroundRevealed = false;

    /** Start downloading the GIF early (while the splash logo + chime play). */
    function preloadBackground() {
        if (!backgroundPreloadPromise) {
            backgroundPreloadPromise = new Promise((resolve) => {
                const img = new Image();
    ...
    /** Fade the GIF background layer in behind the UI. */
    function revealBackground() {
        if (backgroundRevealed) return;
        backgroundRevealed = true;

        const showLayer = () => {
            let layer = document.getElementById('app-background');
            if (!layer) {
                layer = document.createElement('div');
                layer.id = 'app-background';
                layer.setAttribute('aria-hidden', 'true');
                document.body.prepend(layer);
            }
            // Subtle scrim + GIF. Set from JS so the theme path resolves
            // exactly like '_/theme/bmw-logo.png' and '_/theme/bmw-door-chime.m4a'.
            // Remove the linear-gradient line if you want the GIF at full brightness.
            layer.style.backgroundImage =
                `linear-gradient(rgba(10, 22, 40, 0.45), rgba(10, 22, 40, 0.45)), url('${BACKGROUND_SRC}')`;
            ...
            // Double rAF guarantees the opacity transition actually runs
            requestAnimationFrame(() => {
                requestAnimationFrame(() => layer.classList.add('visible'));
            });
        };

        // Wait for the GIF, but never block the reveal longer than 1.5 s
        Promise.race([
            preloadBackground(),
    ...
    function showBmwSplashScreen(onComplete) {
        ...
        splash.style.cssText = `position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: #000000; display: flex; align-items: center; justify-content: center; z-index: 99999; transition: opacity 0.6s ease;`;
        splash.innerHTML = `<style> @keyframes bmwSplashPulse { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.06); opacity: 0.85; } } @keyframes bmwSplashRing { 0% { box-shadow: 0 0 0 0 rgba(129, 196, 255, 0.45), 0 0 0 0 rgba(231, 34, 46, 0.35); } 100% { box-shadow: 0 0 0 40px rgba(129, 196, 255, 0), 0 0 0 70px rgba(231, 34, 46, 0); } } </style> <div style="display:flex; flex-direction:column; align-items:center; gap:1.5rem; animation: bmwSplashRing 2.2s ease-out infinite;"> <img src="_/theme/bmw-logo.png" alt="BMW" style="width:200px; height:auto; animation: bmwSplashPulse 1.8s ease-in-out infinite;"> <div style="color:#81c4ff; font-family:Roboto, Arial, sans-serif; font-size:1.4rem; letter-spacing:0.3em; text-transform:uppercase;">BMA Auto Express</div> </div>`;
        document.body.appendChild(splash);

        // ⭐ NEW: start caching the animated GIF while the logo + chime play
        preloadBackground();
        ...
        const finish = () => {
            if (finished) return;
            finished = true;
            // ⭐ NEW: cross-fade the GIF background in as the splash fades out
            revealBackground();
