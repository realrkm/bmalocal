...
@anvil.server.callable
def getCarRegNoFromClientID(valueID):
    _get_current_user()
    with db_cursor() as cursor:
        query = """
...
def update_job_card_details(
    ...
    with db_cursor() as cursor:
        ...
        # --- Query 3: Check if WorkDone exists, then Update or Insert ---
        
        # 1. Check if the record exists for this JobCardRefID
        cursor.execute(
            "SELECT 1 FROM tbl_workdoneinjobcard WHERE JobCardRefID = %s", 
            (jobcardID,)
        )
        ...
        if record_exists:
            ...
            query3 = "UPDATE tbl_workdoneinjobcard SET WorkDone = %s WHERE JobCardRefID = %s"
            cursor.execute(query3, (workDone, jobcardID))
        else:
            # 2b. If it does NOT exist, INSERT it
            query3 = "INSERT INTO tbl_workdoneinjobcard (JobCardRefID, WorkDone) VALUES (%s, %s)"
            cursor.execute(query3, (jobcardID, workDone))
...
def getPaymentsDetails(paymentID):
    ...
    with db_cursor() as cursor:
        ...
        result = [
            {
                "No": index + 1,
                "Date": row[0].strftime("%d-%m-%Y") if row[0] else None,
                "JobCardRef": row[1],
                "PaymentMode": row[2],
...
def fillReportData(jobCardID, docType, logo_path: str = os.getenv("LOGO"), font_path: str = os.getenv("FONT_PATH")) -> str:
    ...
    for item in reportdetails:
        ...
        if ref in seen_refs:
            ...
            invoice_display = html_module.escape(str(item['InvoiceAmount']))
            seen_refs.add(ref)
        ...
        items_html += f"""
                    <tr class="item-row">
                        ...
                        <td>{html_module.escape(str(ref))}</td>
                        <td>{html_module.escape(str(item['PaymentMode']))}</td>
                        <td>{invoice_display}</td>
            ...
            result.append(
                {
                    ...
                    "Discount": row[5],
                    "MaxOfDate": row[6],
                    "StockBalance": f"{float(row[7] or 0):.2f}",
                }
            )
    ...
    with db_cursor() as cursor:
        ...
        for count, row in enumerate(rows, start=1):
            result.append({
                ...
                "Name": row[0],
                "PartNo": row[1],
                "Cost": f"{float(row[2] or 0):,.2f}" if row[2] is not None else None,
                "Selling": f"{float(row[3] or 0):,.2f}" if row[3] is not None else None,
                "Discount": row[4],
    ...
    with db_cursor() as cursor:
        ...
        for idx, row in enumerate(rows, start=1):
            row_data = {
                ...
                "PartNo": row[2],
                "OrderLevel": row[3],
                "TotalIssued": float(row[4] or 0),
                "LatestStockTake": float(row[5] or 0),
                "TotalStock": float(row[7] or 0),
                "StockBalance": f"{float(row[8] or 0):.2f}",
...
def export_stock_balance(rows):
    ...
    return make_excel(
        headers,
        processed_rows,
        filename=f"export_stock_balance_{today}.xlsx",
...
@anvil.server.callable()
def export_current_parts_and_partno():
    _get_current_user()
    headers = ["No", "Name", "PartNo", "Quantity"]
    ...
    return make_excel(
        headers,
        processed_rows,
        filename="current_parts_and_partno.xlsx",
        sheet_name="Current Parts & PartNo",
    )

@anvil.server.callable()
def export_import_orders(rows):
    _get_current_user()
    
    # Sort rows by OrderDate in descending order
    sorted_rows = sorted(
        rows, 
        key=lambda row: row.get("OrderDate", ""), 
        reverse=True
    )
    ...
    return make_excel(
        headers,
        processed_rows,
        filename=f"export_import_orders_{today}.xlsx",
        sheet_name="Import Orders",
    )
...
def saveBarcodePartNo(barcode, partno):
    ...
    try:
        with db_cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO tbl_barcodepartnomapping (Barcode, PartNo)
                VALUES (%s, %s)
                """,
                (barcode, partno),
            )
        ...
        return {
            "success": True,
            "message": f"Barcode '{barcode}' is mapped to PartNo '{partno}'."
        }
        ...
        if e.errno == 1062:
            return {
                "success": False,
                "message": f"This barcode '{barcode}' is already mapped to PartNo '{partno}'."
            }
...
def _process_stocktake_item(cursor, part_no, quantity):
    quantity=float(quantity)

    """
    Core stocktake logic, reusable by both save_stocktake and import_counting_stock_quantity.
    Expects an already-open cursor.
    Returns True if processed, False if part not found.
    """
    ...
    total_stock=float(total_stock or 0.0)
    total_issued = float(total_issued or 0.0)
    ...
    # 5. Insert/Update tbl_stocktakeharmonized
    cursor.execute("""
        INSERT INTO tbl_stocktakeharmonized (StockTakeDate, CarPartNameID, HarmornizedValue)
        VALUES (CURDATE(), %s, %s)
        ON DUPLICATE KEY UPDATE HarmornizedValue = VALUES(HarmornizedValue)
    """, (carpart_id, variance))

    # 6. Insert/Update tbl_finalcapturedstocktaking
    cursor.execute("""
    ...
    """, (carpart_name, part_number, reorder_level, location, balance, quantity, variance, comment))
    ...
    with db_cursor() as cursor:
        for item in repeating_panel_data:
            try:
                ...
                skipped.append(f"Invalid item {item}: {e}")
                continue

            processed = _process_stocktake_item(cursor, part_no, quantity)
            if not processed:
                skipped.append(f"PartNo '{part_no}' not found in database — skipped.")

    result = "Stocktake saved/updated successfully."
    if skipped:
...
@anvil.server.callable()
def import_counting_stock_quantity(file):
    _get_current_user()
    ...
    # Validate headers
    expected_headers = ["No", "Name", "PartNo", "Quantity"]
    actual_headers = [cell.value for cell in ws[1]]
    if actual_headers != expected_headers:
        raise Exception(
            f"Invalid file format. Expected headers: {expected_headers}, but got: {actual_headers}"
    ...
    with db_cursor() as cursor:
        for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            ...
            # Validate Quantity
            valid, quantity_result = is_valid_number(quantity)
            if not valid:
                if quantity_result == "negative":
                    skipped_rows.append(f"Row {row_idx} ({name}): Quantity cannot be negative ({quantity}) — skipped.")
                else:
                    skipped_rows.append(f"Row {row_idx} ({name}): Quantity is not a valid number — skipped.")
                continue
            ...
            # Process stocktake for this row
            processed = _process_stocktake_item(cursor, str(part_no).strip(), float(quantity_result))
            if not processed:
                skipped_rows.append(f"Row {row_idx} ({name} / {part_no}): Stocktake processing failed — skipped.")
                continue
...
def saveImportOrderTracking(
    dateValue, clientID, partname, partnumber, quantity, brand, status
):
    _get_current_user()
    with db_cursor() as cursor:
        query = """
            INSERT INTO tbl_importordertracking
                (OrderDate, ClientID, PartName, PartNumber, Quantity, Brand, Status)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        cursor.execute(
            query, (dateValue, clientID, partname, partnumber, quantity, brand, status)
        )
...
@anvil.server.callable()
def get_import_orders_for_selection(client_id: int = None, order_date: str = None):
    """
    Return import order rows.

    If client_id and order_date are provided:
    ...
    with db_cursor() as cursor:
        ...
        if client_id is not None:
            conditions.append("tbl_importordertracking.ClientID = %s")
            params.append(client_id)

        if order_date is not None:
            conditions.append("tbl_importordertracking.OrderDate = %s")
        ...
        # Final ordering
        base_sql += " ORDER BY tbl_importordertracking.ID"
...
@anvil.server.callable()
def saveBookingDetails(customer_id, regno, period, details):
    # Ensure the user is logged in before saving
    _get_current_user()
    
    with db_cursor() as cursor:
        query = """
            INSERT INTO tbl_appointmentbooking (ClientID, RegNo, Period, Details, IsSent)
            VALUES (%s, %s, %s, %s, 0)
        """
        try:
            cursor.execute(query, (customer_id, regno, period, details))
            # If your connection doesn't auto-commit, uncomment the next line:
            # cursor.connection.commit()
            return True
        except Exception as e:
            # Log the error and raise it back to the UI if needed
    ...
    with db_cursor() as cursor:
        query = """
            SELECT 
                ...
                b.RegNo,
                b.ID
            FROM tbl_appointmentbooking b
            LEFT JOIN tbl_clientcontacts c ON b.ClientID = c.ID
            WHERE DATE(b.Period) >= CURDATE() AND b.IsSent = 0
            ORDER BY b.Period 
        """
        cursor.execute(query)
        rows = cursor.fetchall()
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if dt_object:
                # Combined format: "27-04-2026 at 11:00 AM"
                d['FormattedSchedule'] = dt_object.strftime("%d-%m-%Y at %I:%M %p")
            
            d['No'] = index
            booking_data.append(d)
            
        return booking_data

@anvil.server.callable()
def updateAppointmentSent(booking_id):
    _get_current_user()
    with db_cursor() as cursor:
        query = """
            UPDATE tbl_appointmentbooking
            SET IsSent = 1
            WHERE ID = %s
        """
