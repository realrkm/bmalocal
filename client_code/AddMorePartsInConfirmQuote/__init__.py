from ._anvil_designer import AddMorePartsInConfirmQuoteTemplate
from anvil import *
import anvil.server
import anvil.users
...
class AddMorePartsInConfirmQuote(AddMorePartsInConfirmQuoteTemplate):
    ...
    def cmbJobCardRef_change(self, **event_args):
        ...
        result = anvil.server.call(
            "getQuotationConfirmationFeedback", self.cmbJobCardRef.selected_value["ID"]
        )
        if result:
            self.repeating_panel_assigned_parts.items = result
        else:
            self.repeating_panel_assigned_parts.items = anvil.server.call(
                "populate_confirmation_from_quote",
                self.cmbJobCardRef.selected_value["ID"],
            )
